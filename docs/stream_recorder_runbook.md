...
- `STREAM_RECORDER_STREAMS`: normalized stream contracts to persist.
...
- `S3_RECORDING_SUBTREES`: comma-separated recording subtrees to upload. For
  the EC2 capture profile this should include
  `canonical,raw-ingest,producer-canonical`. Add `producer-raw` only when the
  optional parser-side raw mirror is enabled.
...
streams. It measures and archives what an external Aeron consumer sees. It is
not the authoritative raw replay source; use `raw-ingest` recordings for
end-to-end replay through cluster ingress.
