...
| `FRONTEND_ADAPTER_SOURCE` | `recording` | `recording` or `aeron` |
...
| `FRONTEND_ADAPTER_RECORDING_MAX_EVENTS` | `0` | optional cap on recording source |
...
FRONTEND_ADAPTER_RECORDING_ROOT=recordings \
...
It mounts `./recordings` read-only and exposes port 8090 on the loopback host.
Use `FRONTEND_ADAPTER_SOURCE=aeron` with the `cluster-live` profile to drive it
from a running tickerplant channel.
...
- No WebSocket streaming. The UDF protocol is HTTP polling; that's sufficient
  for the demo chart.
- No replay session management. To replay history, run with
  `FRONTEND_ADAPTER_SOURCE=recording` against a `recordings/canonical` tree;
  there is no pause/seek API.
