...
check_product_static_ui() {
    ...
    grep -q 'demo-signal-strip' "$index_file"
    grep -q 'demo-signal-throughput' "$index_file"
    grep -q 'product-market-panel' "$index_file"
    grep -q 'trader-monitor-panel' "$index_file"
    grep -q 'distribution-panel' "$index_file"
    grep -q 'distribution-sample-payload' "$index_file"
    grep -q 'trader-bid' "$index_file"
    grep -q 'research-feature-limit' "$index_file"
    grep -q 'research-export-csv' "$index_file"
    grep -q 'runtime-operator-panel' "$index_file"
    grep -q 'runtime-pipeline-status' "$index_file"
    grep -q 'runtime-cursor-lag' "$index_file"
    grep -q 'latency-freshness-panel' "$index_file"
    grep -q 'latency-throughput' "$index_file"
    grep -q 'replay-status-panel' "$index_file"
    grep -q 'operator-e2e-latency' "$index_file"
    grep -q 'operator-pipeline-counts' "$index_file"
    ...
    grep -q '/features?' "$app_file"
    grep -q '/health' "$app_file"
    grep -q '/ops/pipeline' "$app_file"
    grep -q '/ops/latency' "$app_file"
    grep -q '/api/demo/replay/status' "$app_file"
    grep -q 'renderDemoSignal' "$app_file"
    grep -q 'renderDistributionStatus' "$app_file"
    grep -q 'loadReplayStatus' "$app_file"
