...
case "$PRODUCT_DEMO_ORCHESTRATOR_SMOKE,$PRODUCT_DEMO_LIVE_PREFLIGHT_SMOKE" in
    1,*|true,*|TRUE,*|yes,*|YES,*|on,*|ON,*|*,1|*,true|*,TRUE|*,yes|*,YES|*,on|*,ON)
        ...
        if [ "$FRONTEND_ADAPTER_OPERATOR_CONTROL_ENABLED" != "true" ]; then
            printf 'product demo operator smoke requires FRONTEND_ADAPTER_OPERATOR_CONTROL_ENABLED=true\n' >&2
            exit 2
        fi
...
export PRODUCT_DEMO_LIVE_PREFLIGHT_SMOKE PRODUCT_DEMO_LIVE_PREFLIGHT_REQUIRED
...
"$SCRIPT_DIR/product-demo-live-preflight-smoke.sh"
wait_demo_orchestrator_smoke
