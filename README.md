...
Durable live storage is DB-primary: raw websocket input and canonical events are written to Postgres/Timescale through the async DB writer. NDJSON recorders remain available for explicit capture, legacy archive/import, and debug workflows.
...
We use Aeron for low-latency publication and Postgres/Timescale for live query, audit, and replay storage. S3-backed NDJSON remains a cold archive/export path for capture profiles, not the live source of truth.
...
The storage path is split by purpose. Live ingestion writes raw websocket payloads and canonical events to DB tables first. `RawIngestRecorder` and `TickerplantStreamRecorder` remain explicit NDJSON capture tools for recorder soaks, archive/import, and downstream-consumer validation. `HistoricalBackfillCli` records REST backfills and can preserve raw REST responses under `raw-rest`.
...
New live captures store raw websocket input and canonical events primarily in Postgres/Timescale. Raw replay defaults to the DB raw ingest table. NDJSON/S3 recording is retained for `recording-capture`, legacy archive/import, fixtures, and debug exports. The downstream stream recorder remains useful for validating what Aeron clients receive.
...
TimescaleDB is the primary live query and replay store. S3 remains useful for cold archive/export and retained legacy imports without adding more hot-path consumers.
